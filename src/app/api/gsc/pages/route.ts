...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log("Pages API - Session user email:", session.user.email);
    ...
    const pagesData = await gscClient.getPages(siteUrl, {
    ...
    return NextResponse.json(
      { success: false, error: "Failed to fetch pages data" },
      { status: 500 },
    );
  }
}
